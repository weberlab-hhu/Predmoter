...
class PredmoterSequence3(Dataset):
    def __init__(self, h5_files, type_, dsets, seq_len, blacklist, rank, world_size):
        super().__init__()
        self.compressor_x = numcodecs.blosc.Blosc(cname="lz4", clevel=9, shuffle=2)
        self.compressor_y = numcodecs.blosc.Blosc(cname="lz4", clevel=9, shuffle=0)
        self.h5_files = h5_files
        ...
        # todo, maybe preshuffle the coords according to a set seed, that is dependent on a user seed?
        # todo, shuffle per file then split to GPUs?
        # todo, how to make this deterministic when resuming training?
        self.coords = self.get_coords()
        self.shard_length = self.coords.shape[0] // self.world_size  # todo, make more robust later
        self.start_idx = rank * self.shard_length
        self.end_idx = (rank + 1) * self.shard_length
        self.shard_coords = self.coords[self.start_idx:self.end_idx]
        self.x_dtype = np.float16
        self.y_dtype = np.float32
        self.bases = h5py.File(h5_files[0], mode="r")["data/X"].shape[-1]
        self.X = []
        self.Y = []
        if type_ != "test":
            log.info(f"Loading {type_} data shard into memory on rank {rank} ...")
        self.create_dataset()
        log.info(f'shape X: {np.frombuffer(self.compressor_x.decode(self.X[0]), dtype=self.x_dtype).shape}, shape Y: {np.frombuffer(self.compressor_y.decode(self.Y[0]), dtype=self.y_dtype).shape}')
    ...
    def create_dataset(self):
        ...
        # ------------------
        main_start = time.time()
        coord_dict = {key: self.coords[self.coords[:, 0] == key, 1] for key in np.unique(self.coords[:, 0])}

        # Read in data
        # ------------------
        for file_idx in coord_dict.keys():
            h5df = self.h5dfs[file_idx]
            indices = coord_dict[file_idx]
            n = MAX_VALUES_IN_RAM // self.seq_len  # dynamic step for different sequence lengths
            for i in range(0, len(coord_dict[file_idx]), n):  # read in chunks for saving memory (RAM)
                X = np.array(h5df["data/X"][indices[i:i + n]], dtype=self.x_dtype)
                Y = []
                for key in self.dsets:
                    if f"{key}_coverage" in h5df["evaluation"].keys():
                        y = np.array(h5df[f"evaluation/{key}_coverage"][indices[i:i + n]], dtype=self.y_dtype)
                        y = np.around(np.mean(y, axis=2), 2)  # avg of replicates, round to 2 digits
                        Y.append(y)
                    else:
                        Y.append(np.full((X.shape[0], X.shape[1]), -3, dtype=self.y_dtype))
                self.Y += self._encode_one(np.stack(Y, axis=2), "Y")
                self.X += self._encode_one(X, "X")
        mem_size = (sum([sys.getsizeof(x) for x in self.X]) +
                    sum([sys.getsizeof(y) for y in self.Y]))

        log.info(f'Loading {self.type_} data shard on rank {self.rank} into memory took '
                     f'{round((time.time() - main_start) / 60, ndigits=2)}min')
        log.info(f'Mem size of dataset shard on rank {self.rank} in memory '
                     f'{round((mem_size / 1024 ** 3), ndigits=2)}Gb')
...
def get_dataset(h5_files, type_, dsets, seq_len, ram_efficient, blacklist,
    ...
    """
    if distributed:
        return PredmoterSequence3(h5_files, type_, dsets, seq_len, blacklist, rank, world_size)
    else:
        if ram_efficient:
